...
const SHIRTS = JSON.parse(fs.readFileSync(path.join(ROOT, "data", "shirts.json"), "utf8"));
...
function main() {
  ...
  const have = new Set(fs.existsSync(og) ? fs.readdirSync(og).filter((f) => /^mono-\d{4}\.png$/.test(f)).map((f) => f.slice(0, -4)) : []);
  const hasDefault = fs.existsSync(path.join(og, "default.png"));
  ...
  for (const { id, file } of pages) {
    ...
    const price = (byId.get(id)?.price ?? 0).toFixed(2);
    html = html.replace(
      /<meta property="og:type" content="website"\/>/,
      `<meta property="og:type" content="product"/><meta property="product:price:amount" content="${price}"/><meta property="product:price:currency" content="USD"/>`,
    );
  ...
  console.log(`postbuild: ${pages.length} product pages checked${ogRan ? `, all with their og image` : ""}`);
}
