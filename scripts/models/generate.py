...
for j in jobs:
    ...
    for attempt in range(5):
        img = pipe(j["prompt"], negative_prompt=NEG + (", black shirt" if j.get("color") == "white" else ", white shirt"), num_inference_steps=j.get("steps", 6), guidance_scale=1.5, width=512, height=704, generator=torch.Generator().manual_seed(j["seed"] + attempt * 101)).images[0]
        g = img.convert("L").crop((200, 230, 312, 380)); px = list(g.getdata()); m = sum(px) / len(px)
        ok = (m > 105) if j.get("color") == "white" else (m < 80) if j.get("color") == "black" else True
