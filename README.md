...
5. **Privacy**: the saved "last order" keeps only its number, items and totals — never name, email or address (cart store v3 removes them from older saves). A Content-Security-Policy meta tag limits scripts, images and requests to the site itself (plus the API when configured).
6. **Backend hooks** (`lib/api.ts`): nothing is sent anywhere unless `NEXT_PUBLIC_API_URL` is set. Without it, a signup email is remembered on this device only (and the page says so), and features that need shared data (referrals, "most swiped") aren't shown. Store promises (exchanges, shipping time, fabric, returns, fit advice) live in `lib/store-policy.ts`, pending approval.
