...
- `public/prints/print_1.svg` … `print_3400.svg`: 3:4 single-ink SVG prints (drawn ~6 KB, photographs ~18 KB; minified, path data relative where shorter; `scripts/gen/minify.ts`), plus `print_<n>_<colour>.svg` — a photograph's print for its other tee colour (see "Photographs")
...
- `scripts/photos/fetchPhotos.ts` (needs the network, run by hand; downloads cached in `node_modules/.cache/mono-photos`): `candidates` reads the museum metadata from the public bucket (`s3://smithsonian-open-access`) and keeps CC0 images of animals (National Zoo) and of aircraft, engines and instruments (Air and Space); `prep` turns each into a 180×240 grey + alpha PNG — cropped to 3:4, levels stretched, and for an object shot against a studio backdrop the backdrop removed (flood fill that follows the backdrop's gradient and soft shadows), so the object floats on the tee like the drawn prints; `sheet` writes numbered contact sheets; `select` takes the best 200 per category → `data/photos/img/*.png` + `data/photos/photos.json`.
- `scripts/photos/curation.ts` is the hand review of those contact sheets: what was left out (people in the picture — a keeper's hand holding a frog counts —, busy museum halls, lost masks, near-duplicates, text-heavy plates) and subject clean-up (the species rather than the zoo name of the animal, record typos, no personal names in titles).
- The generator (`scripts/gen/photo.ts`) screens each photograph in one ink: a fine 45° halftone, a coarse bold halftone (full-frame pictures only; a cut-out object turns to a blob) or a horizontal line screen. Full-frame pictures fade out at the edges instead of ending in a hard box.
- **Invert.** A drawn print is strictly two-tone, so the other tee colour is its exact CSS inversion. A photograph inverted is a *negative*. So every photo design has **two positive prints**: on a black tee the white ink lays down the picture's lights, on a white tee the black ink lays down its darks (`print_<n>.svg` for its original colour, `print_<n>_<colour>.svg` for the other). `printUrl()` / `needsInvert()` in `lib/catalog.ts` choose; `PrintImage`, the share image and the zoom all go through them. The original colour is the tee that carries more of the picture.
